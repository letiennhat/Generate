...
class Main:
    ...
    def free_name(self,key=None):
        ...
        if key == "F":
            return np.random.choice(provider.first_names_female)+' '+np.random.choice(provider.middle_name)+' '+np.random.choice(provider.last_names)
        elif key == "M":
            return np.random.choice(provider.first_names_male)+' '+np.random.choice(provider.middle_name)+' '+np.random.choice(provider.last_names)
    def free_email(self):
    ...
    def generation(self):
        ...
        return dic
    def jsonload(self):
        json_ = json.dumps(self.generation(),indent=2).encode('utf-8')
