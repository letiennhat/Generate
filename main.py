import numpy as np
from generate_data import provider
import json,time,os
class Main:
    ...
    def free_name(self,key=None):
        ...
        if key == "F":
            return " ".join(np.random.choice(provider.first_names_female,np.random.randint(2)))+' '+np.random.choice(provider.middle_name)+' '+" ".join(np.random.choice(provider.last_names,np.random.randint(1,2)))
        elif key == "M":
            return " ".join(np.random.choice(provider.first_names_male,np.random.randint(2)))+' '+np.random.choice(provider.middle_name)+' '+" ".join(np.random.choice(provider.last_names,np.random.randint(1,2)))
    def free_email(self):
